 ...
 *
 * Отвечает на два вопроса: что и когда с машиной делали и во сколько это
 * тогда обошлось. Цена здесь — память, а не бухгалтерия: в баланс, расходы и
 * аналитику она не попадает, иначе замена масла посчиталась бы дважды —
 * здесь и в операции, которой человек её уже записал.
 */
export interface ServiceRecord {
  ...
  mileage?: number
  /**
   * Сколько стоило в КОПЕЙКАХ на тот момент. Ни на один остаток не влияет —
   * см. комментарий к самой записи.
   */
  cost?: number
  note?: string
 ...
 * Машина.
 *
 * Своего поля пробега у неё нет. Отдельное число пришлось бы править руками
 * каждую неделю, а правили бы его раз в полгода — и на экране висел бы пробег,
 * устаревший на десять тысяч. Сегодняшним считается пробег из последней записи
 * журнала: он вводится там, где его и так смотрят, — на приёмке в сервисе.
 */
export interface Vehicle {
  ...
  vin?: string
  /**
   * Во сколько обошлась покупка, в копейках. Справочное число: в баланс не
   * входит и с ним не спорит — машина дешевеет каждый год, а сумма, вбитая
   * однажды, этого не знает.
   */
  price?: number
...
export interface Property {
  ...
  area?: number
  /**
   * Комнат. Ноль — студия: комнаты в ней не считают, потому что она одна и
   * та же на всё, и «1 комната» про студию сказать так же неверно, как «0».
   */
  rooms?: number
  floor?: number
  /**
   * Стоимость в копейках: за сколько купили или во сколько оценивают.
   * Как и у машины, справочная — в баланс не входит.
   */
  price?: number
  note?: string
